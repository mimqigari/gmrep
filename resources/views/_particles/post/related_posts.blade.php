...
if($related_type == 'category'){
    $postCats = collect($categories)->pluck('category_id')->toArray();
    $related = \App\Post::whereHas('categories', function ($q) use ($postCats) {
        return $q->whereIn('category_id', $postCats);
    });
}else{
    $postTags = collect($tags)->pluck('id')->toArray();
    $related = \App\Post::whereHas('tags', function ($q) use ($postTags) {
        return $q->whereIn('id', $postTags);
    });
}
