...
@php($content = str_replace(array('<font', '/font>' , 'iframe' ,), array('<span', '/span>' , 'amp-iframe' ), $content))
    <?php $html=preg_replace( '/<img .*? src="([^"]*)" .*?>/' , '<amp-img src="$1" width="1.1" height="1" layout="responsive" alt="AMP">
</amp-img>' , $content); $html=preg_replace('/<img(.*?)\/?>/', '<amp-img$1 width="1.1" height="1" layout="responsive"
        alt="AMP">
        </amp-img>' ,
        $html); ?>
        <p>{!!$html!!}
