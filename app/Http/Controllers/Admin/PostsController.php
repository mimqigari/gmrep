        ...
        return Datatables::of($post)
            ...
            ->addColumn('approve', function ($post) {
                if ($post->deleted_at !== null) {
                    ...
                    $fsdfd = '<div class="label label-info">' . trans("admin.DraftPost") . '</div>';
                } elseif ($post->approve == 'no') {
                    $fsdfd = '<div class="label label-info>' . trans("admin.AwaitingApproval") . '</div>';
                } elseif ($post->featured_at !== null) {
                    $fsdfd =  '<div class="clear"></div><div class="label label-featured">' . trans("admin.FeaturedPost") . '</div>';
                ...
                if ($post->published_at->getTimestamp() > Carbon::now()->getTimestamp()) {
                    $fsdfd .= '<div class="label bg-gray ml-5">' . trans('v3.scheduled_date', ['date' => $post->published_at->format('j M Y, h:i A')])  . '</div>';
                }
